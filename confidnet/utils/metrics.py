...
class Metrics:
    ...
    def get_scores(self, split="train"):
        ...
        if 'tpr' in self.metrics:
            tpr = len(self.proba_pred[(self.accurate == 1) & (self.proba_pred >= 0.5)]) / len(
                    self.proba_pred[(self.accurate == 1)]
                )

            scores[f"{split}/tpr"] = {"value": tpr, "string": f"{tpr:05.2%}"}
        if  'tnr' in self.metrics:
            tn_th = 1.0 / self.n_classes
            tnr = len(self.proba_pred[(self.accurate == 1) & (self.proba_pred < tn_th)]) / len(
                    self.proba_pred[(self.accurate == 1)]
                )
            scores[f"{split}/tnr"] = {"value": tnr, "string": f"{tnr:05.2%}"}
        return scores
