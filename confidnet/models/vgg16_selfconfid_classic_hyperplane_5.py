...
class VGG16SelfConfidClassicHyperplane5(AbstractModel):
    def __init__(self, config_args, device):
        ...
        # self.uncertainty_linear = nn.Linear(512, 1, bias=True)
    ...
    def forward(self, x):
        ...
        uncertainty = self.uncertainty5(uncertainty) / (torch.pow(self.uncertainty5.weight, 2).sum().sqrt() + self.temperature)
        pred = self.fc2(out)
